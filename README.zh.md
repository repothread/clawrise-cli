...
- npm 发布后的升级策略、校验增强与 release hardening 仍需继续完善

## npm 安装

如果你希望直接安装预编译版本，可以通过 npm 安装：

```bash
npm install -g clawrise-cli
```

安装当前平台对应的 `clawrise` 预编译二进制，并随包带上第一方 `feishu` / `notion` provider plugin。

如果你关注 npm 分发背后的发布流程、版本分支约定和构建脚本，请查看 [docs/zh/npm-release-workflow.md](docs/zh/npm-release-workflow.md)。
