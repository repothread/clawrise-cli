...
func runAuthInspect(args []string, cfg *config.Config, store *config.Store, stdout io.Writer, manager *pluginruntime.Manager) error {
	if manager == nil {
		return fmt.Errorf("plugin manager is required")
	}
	if len(args) > 1 {
...
func runAuthComplete(args []string, cfg *config.Config, store *config.Store, stdout io.Writer, manager *pluginruntime.Manager) error {
	if manager == nil {
		return fmt.Errorf("plugin manager is required")
	}
