	...
	return adapter.OperationSpec{
		...
		Input: adapter.InputSpec{
			...
			Notes: []string{
				...
				"For navigation or index pages, prefer block-level edits such as `notion.block.append` and `notion.block.delete` when you need to preserve existing child content.",
				"When the goal is to restructure page hierarchy rather than overwrite page body content, prefer `notion.page.move` over using markdown replacement as a workaround.",
			},
			Sample: map[string]any{
				"page_id": "page_demo",
		...
		Examples: []adapter.ExampleSpec{
			{
				Title:   "Replace markdown content without deleting existing child content",
				Command: `clawrise notion.page.markdown.update --dry-run --json '{"page_id":"page_demo","type":"replace_content","replace_content":{"new_str":"# Updated title\n\nGenerated by Clawrise."}}'`,
			},
			{
				Title:   "Explicitly allow a destructive replace when child pages or databases should be removed",
				Command: `clawrise notion.page.markdown.update --dry-run --json '{"page_id":"page_demo","type":"replace_content","replace_content":{"new_str":"# Rebuilt homepage","allow_deleting_content":true}}'`,
			},
		},
	}
}
